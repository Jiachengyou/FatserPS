...
#             head = YOLOXHeadPSTeacher(self.num_classes, self.width, in_channels=in_channels, act=self.act, reid_embedding=self.reid_embedding)
            head = YOLOXHeadPS(self.num_classes, self.width, in_channels=in_channels, strides=[8,16,32], act=self.act, reid_embedding=self.reid_embedding, dataset=self.dataset)
...
#         sampler = InfiniteSampler(len(self.dataset), seed=self.seed if self.seed else 0)
        ...
        sampler = IdentityInstanceSampler(self.dataset.pidIndex, 16)
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False, legacy=False):
        from yolox.evaluators import PSEvaluator, COCOEvaluator
