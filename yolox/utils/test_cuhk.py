...
from yolox.utils import get_max_iou, get_good_iou
...
def test_result_cuhk(all_dets):
    ...
    for i, dets in enumerate(all_dets):
        ...
        gallery_dicts[image_id]['bbox'] = np.array([k['bbox'] for k in dets])
        gallery_dicts[image_id]['scores'] = np.array([k['score'] for k in dets])
        gallery_dicts[image_id]['feats'] = np.array([k['reid_feat'] for k in dets])
    ...
    for thresh in all_thresh:
        if db_name == "psdb_test":
            gallery_size= 100
            test = loadmat(osp.join(root_dir, "../../data/CUHK-SYSU/annotation/test/train_test/TestG{:d}.mat".format(gallery_size)))
