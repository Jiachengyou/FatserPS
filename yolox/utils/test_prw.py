...
# @jit(forceobj=True)
def test_result_prw(all_dets, det_thresh=0.2, gallery_size=-1, ignore_cam_id=True, input_path=None):
    #results_path = '/raid/ljp/code/chao_mmdetection/jobs/dcn_base_focal/'

    # change here
    data_root='../PSDT-main/data/PRW-v16.04.20'
    ...
    for det in all_dets:
        detection = np.array([k['bbox']+[k['score'],] for k in det])
        gallery_det.append(detection)

        feat = np.array([k['reid_feat'] for k in det])
